...
export async function createStaffAction(formData: FormData) {
  ...
  try {
    await db.transaction(async (tx) => {
      ...
      await tx.insert(businessMemberships).values({ businessId: access.business.id, userId: staff.id, role: parsed.data.role });
      ...
      await tx.insert(auditLogs).values({ businessId: access.business.id, userId: access.user.id, action: "user.created", entityType: "user", entityId: staff.id, ipAddress: requestHeaders.get("x-forwarded-for")?.split(",")[0]?.trim() || null, metadata: { role: parsed.data.role, branchIds: assignedBranches.map((branch) => branch.id) } });
    });
  } catch (error) {
...
export async function updateStaffAction(formData: FormData) {
  const access = await requirePermission("team:manage");
  const parsed = updateStaffSchema.safeParse({ memberId: formData.get("memberId"), role: formData.get("role"), branchIds: formData.getAll("branchIds"), active: formData.get("active") === "on" });
  ...
  if (data.memberId === access.user.id && !data.active) redirect(`/team/${data.memberId}/edit?error=You+cannot+deactivate+your+own+membership.`);
  ...
  await db.transaction(async (tx) => {
    await tx.update(businessMemberships).set({ role: data.role, active: data.active, updatedAt: new Date() }).where(and(eq(businessMemberships.businessId, access.business.id), eq(businessMemberships.userId, data.memberId)));
    ...
    await tx.insert(auditLogs).values({ businessId: access.business.id, userId: access.user.id, action: membership.role !== data.role ? "user.role_changed" : data.active ? "user.access_updated" : "user.deactivated", entityType: "user", entityId: data.memberId, metadata: { oldRole: membership.role, newRole: data.role, active: data.active, branchIds: assignedBranches.map((branch) => branch.id) } });
  });
  revalidatePath("/team");
  redirect("/team");
}
