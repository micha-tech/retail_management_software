...
export const productSchema = z.object({ name: z.string().trim().min(2).max(200), sku: z.string().trim().min(1).max(80).transform((v) => v.toUpperCase()), barcode: z.string().trim().max(100).optional(), categoryId: z.union([z.literal(""), z.uuid()]).optional(),   description: z.string().trim().max(1000).optional(), sellingPrice: moneyInput, costPrice: z.union([z.literal(""), moneyInput]).optional(), unit: z.string().trim().min(1).max(30), minimumStockLevel: z.coerce.number().int().min(0).max(1_000_000), trackInventory: z.string().optional() });
