...
export async function createProductAction(formData: FormData) {
  ...
  try {
    await db.transaction(async (tx) => {
      const [product] = await tx.insert(products).values({ businessId: access.business.id, name: parsed.data.name, sku: parsed.data.sku, barcode: parsed.data.barcode || null, categoryId: parsed.data.categoryId || null, description: parsed.data.description || null, sellingPrice: parseMoney(parsed.data.sellingPrice), costPrice: parsed.data.costPrice ? parseMoney(parsed.data.costPrice) : 0n, unit: parsed.data.unit, minimumStockLevel: parsed.data.minimumStockLevel, trackInventory: parsed.data.trackInventory === "on" }).returning({ id: products.id });
...
export async function updateProductAction(formData: FormData) {
  ...
  try {
    await db.transaction(async (tx) => {
      await tx.update(products).set({ name: data.name, sku: data.sku, barcode: data.barcode || null, categoryId: data.categoryId || null, description: data.description || null, sellingPrice: parseMoney(data.sellingPrice), costPrice: data.costPrice ? parseMoney(data.costPrice) : 0n, unit: data.unit, minimumStockLevel: data.minimumStockLevel, trackInventory: data.trackInventory === "on", active: data.active === "on", updatedAt: new Date() }).where(and(eq(products.id, data.productId), eq(products.businessId, access.business.id)));
