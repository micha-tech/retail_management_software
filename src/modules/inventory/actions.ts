...
export async function receiveStockAction(formData: FormData) {
  ...
  const access = await requireBranchAccess(parsed.data.branchId); if (!["OWNER", "ADMIN", "BRANCH_MANAGER", "STOREKEEPER"].includes(access.role)) redirect("/overview");
...
export async function adjustStockAction(formData: FormData) { const parsed = adjustSchema.safeParse(Object.fromEntries(formData)); if (!parsed.success) redirect("/inventory/adjust?error=Check+the+adjustment+details."); const access = await requireBranchAccess(parsed.data.branchId); if (!["OWNER", "ADMIN", "BRANCH_MANAGER", "STOREKEEPER"].includes(access.role)) redirect("/overview"); await adjustStock({ businessId: access.business.id, branchId: access.branch.id, productId: parsed.data.productId, userId: access.user.id, newQuantity: parsed.data.newQuantity, reason: parsed.data.reason }); revalidatePath("/inventory"); redirect("/inventory"); }
