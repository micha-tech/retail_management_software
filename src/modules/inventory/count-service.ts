...
import { auditLogs, branchInventory, inventoryCountItems, inventoryCounts, operationalAlerts, posSessions, products, stockMovements } from "@/db/schema";
...
export async function postInventoryCount(input: { businessId:string;branchId:string;countId:string;userId:string }) {
  return db.transaction(async (tx) => {
    ...
    if(adjustedItems>0)await tx.insert(operationalAlerts).values({businessId:input.businessId,branchId:input.branchId,type:"INVENTORY_VARIANCE",severity:adjustedItems>=10?"HIGH":"MEDIUM",entityType:"inventory_count",entityId:input.countId,title:`Inventory variance requires review: ${locked[0].count_number}`,reason:`${adjustedItems} product${adjustedItems===1?"":"s"} differed between system and physical count. Net unit adjustment: ${netAdjustment}.`,recommendation:"Review the posted count, supporting notes, and correction movements. Investigate process issues before making further adjustments.",supportingData:{countNumber:locked[0].count_number,adjustedItems,netAdjustment},createdBy:input.userId}).onConflictDoNothing();
