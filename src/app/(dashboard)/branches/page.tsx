...
export default async function BranchesPage() {
  ...
  return <><header className="topbar"><div><p className="eyebrow">Configuration</p><h1>Branches</h1><p>Operational locations in {access.business.name}.</p></div>{hasPermission(access.role, "branch:manage", access.permissions) && <Link className="button primary inline-button" href="/branches/new"><Plus size={17}/> New branch</Link>}</header><main className="page"><section className="surface table-surface"><table><thead><tr><th>Branch</th><th>Code</th><th>Timezone</th><th>Contact</th><th>Status</th></tr></thead><tbody>{records.map((branch) => <tr key={branch.id}><td>{hasPermission(access.role,"branch:manage",access.permissions)?<Link href={`/branches/${branch.id}/edit`}><strong>{branch.name}</strong></Link>:<strong>{branch.name}</strong>}<small><MapPin size={13}/>{branch.address || "No address provided"}</small></td><td><code>{branch.code}</code></td><td>{branch.timezone}</td><td>{branch.email || branch.phone || "—"}</td><td><span className={branch.active ? "pill active" : "pill"}>{branch.active ? "Active" : "Inactive"}</span></td></tr>)}</tbody></table></section></main></>;
}
