...
export function PosTerminal({ products, banks, branchId, sessionId, currency }: { products: Product[]; banks: PaymentBank[]; branchId: string; sessionId: string; currency: string }) {
  ...
  const add = (id: string) => {
    if (saleCompleted) { setKey(crypto.randomUUID()); setCart({ [id]: 1 }); setDiscountInput("0.00"); setPaymentMethod("CASH"); setPaymentBankId(""); return; }
  ...
  const bankTransferValid = paymentMethod !== "BANK_TRANSFER" || paymentBankId !== "";
